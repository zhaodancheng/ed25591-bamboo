...
import (
	...
	"github.com/gitferry/bamboo/piphotstuff"
...
func NewReplica(id identity.NodeID, alg string, isByz bool) *Replica {
	...
	switch alg {
	case "piphotstuff":
		r.Safety = piphotstuff.NewHotStuff(r.Node, r.pm, r.Election, r.committedBlocks, r.forkedBlocks)
	case "hotstuff":
...
func (r *Replica) handleQuery(m message.Query) {
	...
	r.thrus += fmt.Sprintf("Time: %.1f s. Throughput: %6.0f   txs/s,Request rate is %6.0f   txs/s\n", time.Now().Sub(r.startTime).Seconds(), float64(r.totalCommittedTx)/time.Now().Sub(r.tmpTime).Seconds(), requestRate)
